...
/***************************************************************************************************************************************************************/
  ...
  window.plugin.ssp.loadList = function(typeList) {
    ...
    for(var idFolders in list) {
      ...
      if(idFolders != window.plugin.ssp.KEY_OTHER_BKMRK) { element += elementTemp; }
...
/***************************************************************************************************************************************************************/
  ...
  plugin.ssp.addPortalBookmark = function(guid, latlng, label) {
    ...
    window.plugin.ssp.bkmrksObj['portals'][window.plugin.ssp.KEY_OTHER_BKMRK]['bkmrk'][ID] = {"guid":guid,"latlng":latlng,"label":label};
...
//      $('#bookmarksBox.mobile #topBar').prepend(window.plugin.ssp.htmlCallSetBox+window.plugin.ssp.htmlCalldrawBox); // wonk in progress
    ...
    if(window.plugin.ssp.statusBox['show'] === 0) { window.plugin.ssp.switchStatusBkmrksBox(0); }
    if(window.plugin.ssp.statusBox['page'] === 1) { $('#bookmarksBox h5.bkmrk_portals').trigger('click'); }
